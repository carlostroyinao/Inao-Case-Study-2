...
const handleEditSubmit = async (e) => {
  ...
  const handleExportData = () => {
    ...
    const headers = [
      "ID", "Last Name", "First Name", "M.I.", "Birthday", "Age", "Sex",
      "Civil Status", "Purok", "Occupation", "Annual Income",
      "Contact Number", "Religion", "Nationality", "Type of Housing"
    ];
  ...
  const generateQR = (resident) => {
    ...
    console.log("QR Payload:", qrValue);

    const fullName = `${resident.last_name}_${resident.first_name}_${resident.middle_initial}`.replace(/\s+/g, '_');
    ...
    // Wait for rendering before taking screenshot
    setTimeout(() => {
      const qrNode = container.querySelector('#qrCode');
      if (!qrNode) {
      ...
      html2canvas(qrNode).then((canvas) => {
        ...
        const link = document.createElement("a");
        link.href = jpegUrl;
        link.download = `qr_${fullName}.jpg`;
        link.click();
    ...
    }, 500); // Delay ensures QR renders first
  };
                ...
                <div class="recentOrders">
                    <div class="cardHeader">
                        ...
                        <div>
                          ...
                          <button className="btn" onClick={handleExportData}>Export Data</button>
                        </div>
                    </div>
                            ...
                            .map((resident) => (
                              <tr key={resident.id}>
                                ...
                                <td>
                                  ...
                                  <button onClick={() => generateQR(resident)} className="btn">Generate QR</button>
                                </td>
                              </tr>
                            ))}
                        </tbody>
